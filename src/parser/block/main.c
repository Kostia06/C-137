...
NodeBlock** create_blocks(Node** nodes,size_t size,char* scope,size_t* return_size){
    ...
    while(index < size){
        ...
        if(current_node->type == NEW_LINE || current_node->type == SEMICOLON || (index < size && nodes[index]->type == FUNCTION_START)){
            ...
            hold_size = 0;
            type = EMPTY;
            next_spacing = (index < size && nodes[index]->type == FUNCTION_START) ? *(int*)nodes[index]->value : *(int*)current_node->value;
            continue;
        }
        if(current_node->type == FUNCTION_START || current_node->type == FUNCTION_END){
